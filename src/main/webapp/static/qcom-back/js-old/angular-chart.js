(function (factory) {
  ...
  function ChartJsFactory (ChartJs, $timeout) {
        ...
        if(type == 'Line' || type =='Bar'){
            //console.log(type);
              while (colours.length < scope.data.length) {
                colours.push(scope.getColourline());
              }
            return colours.map(convertColourline);
          }
        else{
      while (colours.length < scope.data.length) {
    ...
    function convertColour (colour) {
      ...
      function convertColourline (colour) {
      if (typeof colour === 'object' && colour !== null) return colour;
      if (typeof colour === 'string' && colour[0] === '#') return getColourline(hexToRgb(colour.substr(1)));
        console.log(type);
      return getRandomColourline();
    }
    ...
    function getRandomColour () {
      ...
      function getRandomColourLine () {
      var colour = [getRandomInt(0, 255), getRandomInt(0, 255), getRandomInt(0, 255)];
      return getColourline(colour);
    }
    ...
    function getColourline (colour) {
       // console.log(colour);
      return {
        fillColor: rgba(colour, 0.8),
        strokeColor: rgba(colour, 0),
