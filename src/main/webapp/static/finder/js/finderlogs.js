...
var networkTree = {
    ...
    loadLogs:function(){
    		...
    		link = '/facesix/rest/beacon/ble/networkdevice/finder_log?duration='+dur_val+"&cid="+urlObj.cid
    		 
    		 var searchField = $('#searchTerm').val();
            searchField      = searchField.trim();
           
            if(searchField.trim() == "" || searchField == undefined){
...
$("#exportlog").click(function () {
 ...
 window.open('data:application/vnd.ms-excel,' + encodeURIComponent(str));
	*/
	...
	console.log("searchTerm " +searchTerm +" days " +days + " cid " +cid)
	
	var url = "/facesix/rest/beacon/trilaterationReports/finder_logs_csv?cid="+cid+"&searchTerm="+searchTerm+"&duration="+days
    window.open(url);
