 ...
 * --debug    : Launch  with node-inspector.
 * --debug-brk: Launch  and break on 1st line with node-inspector.
...
gulp.task('vet', function () {
    ...
    //.pipe($.jscs()); //for crying out loud this stuff is strict!!!
...
function generateEnvironmentConstant(env) {
    log('env: ' + env);
    log('using environments config json file: ' + config.environmentsJson);
    log('writing environments config to: ' + config.clientApp);
    var myConfig = require(config.environmentsJson);
    var envConfig = myConfig[env];
    var version = args.version ? args.version : '3.0.0.0';
    log('version: ' + version);
    log('config: ' + envConfig);
    return ngConstant({
        name: 'app',
        deps: false,
        constants: envConfig,
...
gulp.task('build-local', ['environment-local', 'optimize', 'images', 'fonts'], function () {
    build();
});
...
gulp.task('build-test', ['environment-test', 'optimize', 'images', 'fonts'], function () {
    build();
});
...
function build() {
    ...
    log('moving web.config to build folder');
    gulp.src(config.webConfigFile).pipe(gulp.dest(config.build));

    log('moving browsercompatability.html to build folder');
    gulp.src(config.browsercompatibility).pipe(gulp.dest(config.build));
}
...
gulp.task('environment-test', function () {
    generateEnvironmentConstant('test');
});
 ...
 * --nosync
 */
gulp.task('serve-build-local', ['build-local'], function () {
    serve(false /*isDev*/);
});
...
gulp.task('serve-build-test', ['build-test'], function () {
    serve(false /*isDev*/);
});
...
gulp.task('optimize', ['inject'], function () {
    ...
    var assets = $.useref.assets({ searchPath: './' }, lazypipe().pipe(sourcemaps.init, { loadMaps: true }));
    ...
    return gulp
        ...
        .pipe($.uglify()).on('error', function(e){
            console.log("Uglify Error App: " + e.message);
            console.log(e);
         })
        .pipe(getHeader())
        ...
        .pipe(jslibFilter)
        .pipe($.uglify()).on('error', function(e){
            console.log("Uglify Error Lib: " + e.message);
            console.log(e);
...
function serve(isDev, specRunner) {
    log('serving application with isDev: ' + isDev);
